...
/// API服务，处理与DeepSeek和Tripo3D的通信
class APIService {
    static let shared = APIService()
    
    // 从 Info.plist 读取 API 密钥（避免硬编码）
    private let deepSeekAPIKey: String
    private let tripoAPIKey: String
    
    private let deepSeekBaseURL = "https://api.deepseek.com/v1/chat/completions"
    private let tripoBaseURL = "https://api.tripo3d.ai/v2/openapi"
    
    private init() {
        // 从 Info.plist 读取 API 密钥
        guard let deepSeekKey = Bundle.main.object(forInfoDictionaryKey: "DeepSeekAPIKey") as? String,
              let tripoKey = Bundle.main.object(forInfoDictionaryKey: "TripoAPIKey") as? String else {
            fatalError("❌ API keys not found in Info.plist. Please add DeepSeekAPIKey and TripoAPIKey to Info.plist")
        }
        
        self.deepSeekAPIKey = deepSeekKey
        self.tripoAPIKey = tripoKey
    ...
    func generate3DModel(prompt: String) async throws -> String {
        print("🎨 Starting 3D model generation with prompt: \(prompt.prefix(50))...")
        
        // Tripo3D API v2 格式 - 尝试请求 USDZ 格式（visionOS 最佳支持）
        var requestBody: [String: Any] = [
            "type": "text_to_model",
            "prompt": prompt
        ]
        
        // 尝试添加 format 参数请求 USDZ 格式
        // 如果 API 不支持此参数，会返回错误，我们会在错误处理中移除它
        requestBody["format"] = "usdz"
        
        print("📤 Creating Tripo3D task requesting USDZ format (optimal for visionOS)")
        print("💡 If format parameter is not supported, will fall back to default format")
        
        // 使用 JSONSerialization 因为结构不同
        ...
        guard let httpResponse = response as? HTTPURLResponse else {
            print("❌ Tripo API Error: Invalid response type")
            throw APIError.invalidResponse
        }
